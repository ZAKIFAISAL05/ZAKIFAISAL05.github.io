...
async function submitAddGame() {
  ...
  for (const p of platforms) {
    try {
      ...
      showAlert(`URL platform "${p.name}" tidak valid. Gunakan format https://...`, 'error');
      return;
    }
  }
  ...
  btn.disabled    = true;
  btn.innerHTML   = '<i class="fas fa-spinner fa-spin"></i> ' + (editingGameId ? 'Menyimpan...' : 'Menambahkan...');
...
async function renderGameTable() {
  ...
  tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:var(--text-muted);padding:24px;">Memuat data...</td></tr>';
  const games = await fetchGames(true);
  if (!games.length) {
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:var(--text-muted);padding:32px;font-weight:600;">Belum ada game di katalog.</td></tr>';
    return;
  }
  tbody.innerHTML = games.map((g, i) => `<tr data-id="${escHtml(g.id)}" draggable="true" class="drag-row" style="cursor:grab;">
    <td style="color:var(--text-muted);font-size:0.8em;text-align:center;" title="Drag untuk ubah urutan"><i class="fas fa-grip-vertical"></i></td>
  ...
  // Init search filter & drag-drop setiap setelah render
  initGameSearch();
  initDragDropReorder();
}
...
/* ════════════════════════════════════════
   DRAG & DROP REORDER GAME TABLE
   ════════════════════════════════════════ */
let _dragSrcRow = null;

function initDragDropReorder() {
  const tbody = document.getElementById('gameTableBody');
  if (!tbody) return;
  ...
  tbody.querySelectorAll('.drag-row').forEach(row => {
    ...
    row.addEventListener('drop', async e => {
      ...
      const allRows = [...tbody.querySelectorAll('.drag-row')];
      const srcIdx  = allRows.indexOf(_dragSrcRow);
      const tgtIdx  = allRows.indexOf(row);
      if (srcIdx < tgtIdx) tbody.insertBefore(_dragSrcRow, row.nextSibling);
      else                  tbody.insertBefore(_dragSrcRow, row);
      // Rebuild nomor urut
      tbody.querySelectorAll('.drag-row').forEach((r, i) => {
        r.cells[1].textContent = String(i+1).padStart(2,'0');
      });
      // Kirim urutan baru ke API
      const newOrder = [...tbody.querySelectorAll('.drag-row')].map(r => r.dataset.id);
      const reordered = newOrder.map(id => (_gamesCache||[]).find(g => g.id === id)).filter(Boolean);
      try {
        await apiReorderGames(reordered);
        addLog('REORDER — urutan game diperbarui');
        showToast('Urutan game berhasil disimpan!', 'ok');
      } catch(err) {
        showAlert('Gagal simpan urutan: ' + err.message, 'error');
        addLog('REORDER ERROR: ' + err.message, 'err');
        await renderGameTable(); // rollback tampilan
      }
    });
  });
}
...
/* ════════════════════════════════════════
   PREVIEW GAME CARD (sebelum simpan)
   ════════════════════════════════════════ */
function showGamePreview() {
  ...
  const desc      = document.getElementById('fg-desc').value.trim() || 'Deskripsi game akan tampil di sini.';
  const platforms = collectPlatforms();
  const icon      = pendingIconDataUrl || '';

  let existing = document.getElementById('game-preview-modal');
  ...
  modal.innerHTML = `
    <div style="background:var(--bg-card);border:1px solid var(--border-dim);border-radius:8px;padding:24px;max-width:380px;width:90%;font-family:var(--font-body);position:relative;">
      ...
      <div style="display:flex;gap:14px;align-items:flex-start;">
        <div style="width:64px;height:64px;border-radius:8px;overflow:hidden;background:var(--bg-deep);flex-shrink:0;display:flex;align-items:center;justify-content:center;">
          ${icon ? `<img src="${icon}" style="width:100%;height:100%;object-fit:cover;">` : `<i class="fas fa-gamepad" style="font-size:1.8em;color:var(--text-muted);"></i>`}
        </div>
        <div style="flex:1;min-width:0;">
          <div style="font-weight:700;font-size:1.05em;color:var(--text-main);">${escHtml(title)}</div>
          <div style="margin:4px 0;"><span class="badge ${GENRE_BADGE[genre]||'custom-badge'}" style="font-size:0.75em;">${escHtml(genre)}</span></div>
          <div style="font-size:0.8em;color:var(--text-muted);line-height:1.5;">${escHtml(desc.slice(0,120))}${desc.length>120?'…':''}</div>
        </div>
      </div>
      ${platforms.length ? `<div style="margin-top:14px;display:flex;flex-wrap:wrap;gap:6px;">${platforms.map(p=>`<a href="${escHtml(p.url)}" target="_blank" style="font-size:0.78em;background:var(--bg-deep);color:var(--c-cyan);border:1px solid var(--border-dim);padding:4px 10px;border-radius:4px;text-decoration:none;">${escHtml(p.name)}</a>`).join('')}</div>` : ''}
      <div style="margin-top:16px;font-size:0.72em;color:var(--text-muted);text-align:center;">Ini adalah preview tampilan card di website</div>
    </div>`;
  document.body.appendChild(modal);
  modal.addEventListener('click', e => { if (e.target === modal) modal.remove(); });
}

/* ── AUTO-SLUG dari judul game ── */
function initAutoSlug() {
  const titleInput = document.getElementById('fg-title');
  if (!titleInput || titleInput._slugInited) return;
  ...
  titleInput.addEventListener('input', () => {
    ...
    if (!slugEl) {
      ...
      slugEl.id = 'auto-slug-display';
      slugEl.style.cssText = 'font-size:0.75em;color:var(--text-muted);margin-top:4px;font-family:var(--font-title);';
      titleInput.parentNode.appendChild(slugEl);
    }
    slugEl.textContent = slug ? `ID: ${slug}` : '';
  });
}

/* ════════════════════════════════════════
   FILTER TIKET (bug/saran, aktif/selesai)
   ════════════════════════════════════════ */
let _ticketFilterType   = '';  // 'bug' | 'saran' | ''
let _ticketFilterStatus = '';  // 'aktif' | 'selesai' | ''

function initTicketFilter() {
  const panel = document.getElementById('reports-panel');
  if (!panel) return;
  let toolbar = document.getElementById('ticket-filter-toolbar');
  if (toolbar) return; // sudah ada

  toolbar = document.createElement('div');
  toolbar.id = 'ticket-filter-toolbar';
  toolbar.style.cssText = 'display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px;';
  toolbar.innerHTML = `
  ...
  document.getElementById('tf-type').addEventListener('change', e => {
    _ticketFilterType = e.target.value; filterTickets();
  });
  document.getElementById('tf-status').addEventListener('change', e => {
    _ticketFilterStatus = e.target.value; filterTickets();
  });
}
...
/* ════════════════════════════════════════
   STATISTIK LAPORAN DI DASHBOARD
   ════════════════════════════════════════ */
function updateReportStats(tickets) {
  const total    = tickets.length;
  const active   = tickets.filter(t => !t.done).length;
  const done     = tickets.filter(t =>  t.done).length;
  const bugs     = tickets.filter(t => (t.type||'').toLowerCase().includes('bug')).length;
  const bugsAct  = tickets.filter(t => (t.type||'').toLowerCase().includes('bug') && !t.done).length;
